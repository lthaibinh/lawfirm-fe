...
const matchPath = ({ path, end = false }, currentPath) => {
  ...
  if (end && pathSegments.length !== currentSegments.length) return false;
  
  for (let i = 0; i < pathSegments.length; i++) {
    if (i >= currentSegments.length) return false;
    if (pathSegments[i] !== currentSegments[i] && !pathSegments[i].startsWith(':')) return false;
  }
